        ...
        if (choice == "1") {
            ...
            val count = run {
                while (true) {
                    ...
                    if (input != null && (input <= (currentPlayer.hand.size))) {
                        return@run input
                    }
...
class Merchant : ICharacter {
    ...
    override fun ability(
        ...
        currentPlayer.gold += (bonus + 1)
        println("Купец получает ${bonus + 1} золота (бонус роли + кварталы).")
    }
}
    ...
    override fun ability(
        ...
        if (answ== "Y") {
            ...
            val targetDistrict = run {
                while (true) {
                    val input = ConsoleInput().readInput()
                    val card = targetPlayer?.city?.find { it.name.equals(input, true) }
            ...
            targetPlayer?.destroyBuilding(targetDistrict as IDistrict)
